...
st.title("WIR → ITP Activity Tracking Tool (High Performance)")
...
if wir_file and itp_file and activity_file:
    if st.button("Start Processing"):
        ...
        # Fast split multi-activity titles
        st.info("Expanding multi-activity WIR titles (fast)...")
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if not audit_df.empty:
                audit_df.to_excel(writer, index=False, sheet_name='Audit_Unmatched')
        output.seek(0)
